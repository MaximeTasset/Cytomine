...
import cytomine
from cytomine.pyxit.data import build_from_dir
from cytomine.pyxit.estimator import _get_image_data, _partition_images
from cytomine import cytomine, models
from cytomine.utilities.wholeslide import WholeSlide
from cytomine.utilities.objectfinder import ObjectFinder
from cytomine.utilities.reader import Bounds, CytomineReader
from cytomine.utilities.utils import Utils
            ...
            if np.count_nonzero(roi_mask):
                ...
                if (((tilestddev[0] < mindev) and (tilestddev[1] < mindev) and (tilestddev[2] < mindev)) or ((tilemean[0] > maxmean) and (tilemean[1] > maxmean) and (tilemean[2] > maxmean))):
                    ...
                    for i in range(0,width):
                        for j in range(0,pyxit_target_height/2):
                            votes_class[i,j,:] = [1,0]
                        for j in range(height - pyxit_target_height/2,height):
                            votes_class[i,j,:] = [1,0]



                    for j in range(0, height):
                        for i in range(0, pyxit_target_width / 2):
                            votes_class[i,j,:] = [1,0]
                        for i in range(width - pyxit_target_width/2, width):
                            votes_class[i,j,:] = [1,0]
                    ...
                    for geometry in geometries:
                        ...
                        while(not uploaded):
                            try:
                                ...
                                print("socket timeout add_annotation")
                                time.sleep(1)
                                continue
                            except socket.error:
                                print("socket error add_annotation")
                                time.sleep(1)
                                continue
                        endsingle = time.time()
                        ...
                        if annotation:
                            ...
                            while(not termed):
                                try :
                                    ...
                                    print("socket timeout add_annotation_term")
                                    time.sleep(1)
                                    continue
                                except socket.error:
                                    print("socket error add_annotation_term")
                                    time.sleep(1)
                                    continue
                            endsingle = time.time()
        ...
        if parameters['cytomine_postproc']:
            ...
            while(not retrieved):
                try:
                    ...
                    print("socket timeout get_annotations")
                    time.sleep(1)
                    continue
                except socket.error:
                    print("socket error get_annotations")
                    time.sleep(1)
                    continue
