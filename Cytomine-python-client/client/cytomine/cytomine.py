...
#        httplib2.debuglevel=4
	...
	#Maxime Tasset
    def get_pixel_spectre(self, id_image_groupe, width, height):
        spectre = Spectre()
        spectre.id = id_image_groupe
        spectre.width = int(width)
        spectre.height = int(height)
        return self.fetch(spectre)

    def get_rectangle_spectre(self, id_image_groupe, width, height,rwidth, rheight):
        spectre = SpectreCollection()
        spectre.id = id_image_groupe
        spectre.width = int(width)
        spectre.height = int(height)
        spectre.rwidth = int(rwidth)
        spectre.rheight = int(rheight)
        return self.fetch(spectre)

    def get_image_groups(self,id_project):
        imagegroupes = ImageGroupCollection()
        imagegroupes.project = id_project
        return self.fetch(imagegroupes)

    def get_project_terms(self,id_project):
        terms = TermsFromProjectCollection()
        terms.project = id_project
        return self.fetch(terms)

    def get_imageGroupHDF5(self,id_imagegroup):
        imagegroupHDF5 = ImageGroupHDF5()
        imagegroupHDF5.group = id_imagegroup
        return self.fetch(imagegroupHDF5)

    def get_imageSequence(self,id_imagegroup):
        imagesequence = ImageSequence()
        imagesequence.imagegroup = id_imagegroup
        return self.fetch(imagesequence)
